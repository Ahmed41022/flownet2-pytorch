...
if __name__ == '__main__':
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if exists(args.training_dataset_root):
            ...
            block.log('Training Input: {}'.format(
                ' '.join([str([d for d in x.size()]) for x in train_dataset[0][0]])))
            block.log('Training Targets: {}'.format(
                ' '.join([str([d for d in x.size()]) for x in train_dataset[0][1]])))
        ...
        if exists(args.validation_dataset_root):
            ...
            block.log('Validation Input: {}'.format(
                ' '.join([str([d for d in x.size()]) for x in validation_dataset[0][0]])))
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            if args.cuda:
                data, target = [d.cuda() for d in data], [t.cuda()
                                                          for t in target]
            data, target = [Variable(d) for d in data], [
                Variable(t) for t in target]
    ...
    for epoch in progress:
        ...
        if not args.skip_training:
            train_loss, iterations = train(args=args, epoch=epoch, start_iteration=global_iteration, data_loader=train_loader,
                                           model=model_and_loss, optimizer=optimizer, logger=train_logger, offset=offset)
        ...
        train_logger.add_scalar('seconds per epoch',
