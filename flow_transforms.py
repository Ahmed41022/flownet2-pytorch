...
class Scale(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[0] = ndimage.interpolation.zoom(
            inputs[0], ratio, order=self.order)
        inputs[1] = ndimage.interpolation.zoom(
            inputs[1], ratio, order=self.order)
...
class RandomTranslate(object):
    ...
    def __call__(self, inputs, target):
        ...
        x1, x2, x3, x4 = max(0, tw), min(
            w + tw, w), max(0, -tw), min(w - tw, w)
        y1, y2, y3, y4 = max(0, th), min(
            h + th, h), max(0, -th), min(h - th, h)
